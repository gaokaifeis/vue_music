          ...
          <transition-group name="list" tag="ul">
            <li class="item" v-for="(item, index) in sequenceList" :key="item.id" @click="selectItem(item, index)" ref="listItem">
              ...
              <span class="like" @click.stop="toggleFavorite(item)">
                <i :class="getFavariteIcon(item)"></i>
              </span>
              <span @click.once="deleteOne(item)" @click.stop class="delete">
...
export default {
  ...
  methods: {
    ...
    selectItem (item, index) {
      if (this.mode === playMode.random) {
        index = this.playlist.finIndex((song) => {
  ...
  watch: {
    currentSong (newSong, oldSong) {
      if (!this.showFlag || newSong.id === oldSong.if) {
        return
      }
